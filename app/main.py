...
from .models import StoreRequest, QueryRequest, ToolRegistrationRequest
from .services.postgres_client import get_postgres_client, PostgresClient
from .services.qdrant_client import get_qdrant_client, QdrantMemoryClient
from .services.redis_client import get_redis_client, RedisClient
...
async def store_memory(
    ...
    try:
        ...
        if point_id is None:
            ...
            # Note: In a production system, this should be handled with proper transactions
            raise HTTPException(
                status_code=500,
                detail="Failed to store embedding in Qdrant"
            )
        ...
        # Step 4: Return the PostgreSQL ID
        return {
            "success": True,
            "memory_id": memory_id,
            "point_id": point_id,
...
async def query_memory(
    ...
    try:
        ...
        memory_ids = [result["memory_id"] for result in search_results if result["memory_id"]]
        
        # Step 3: Query PostgreSQL for tools that match the query context (Tool Discovery Logic)
        ...
        response = {
            ...
            "tools": {
                "count": len(relevant_tools),
                "results": relevant_tools
            },
            "search_metadata": {
