...
class Memory(Base):
    ...
    memory_metadata = Column(JSON, nullable=True)  # Renamed from 'metadata' to avoid SQLAlchemy conflict
...
class PostgresClient:
    ...
    def store_memory(self, content: str, metadata: Optional[Dict[str, Any]] = None, 
                    embedding_id: Optional[str] = None) -> Optional[int]:
        """
    ...
    def register_tool(self, name: str, description: str, usage: str, 
    ...
    def get_tools_by_context(self, query_context: str, limit: int = 10) -> List[Dict[str, Any]]:
        ...
        try:
            with self.get_session() as session:
                ...
                tools = session.query(RegisteredTool).filter(
